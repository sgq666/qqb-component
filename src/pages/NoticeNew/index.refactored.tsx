...
const NoticeNew: React.FC = () => {
  ...
  // 检查是否有新任务
  const checkForNewTasks = useCallback(
    async () => {
      try {
        console.log("📡 检查新任务中...");
        ...
        const validDepartments = (departments as string[]).filter((dept: string) => dept && dept.trim() !== '');
        const deptCodes = validDepartments.length > 0 
          ? validDepartments 
          : (currentUser?.deptCode ? [currentUser.deptCode] : []);
  ...
  const startListening = useCallback(
    (intervalValue?: number) => {
      // 获取表单数据
      const formData = form.getFieldsValue();
      const { businessTopics = [], intervalMinutes: formIntervalMinutes } = formData as { 
      ...
      const effectiveInterval =
        ...
        formIntervalMinutes !== undefined ? formIntervalMinutes : 
        intervalMinutes;
  ...
  const loadDepartments =
    useCallback(async (): Promise<ExtendedDataNode[]> => {
      try {
        ...
        if (currentUser && currentUser.deptCode) {
          ...
          if (userDept) {
            ...
            if (userDeptLevel <= Number(deptLevel)) {
              ...
              while (currentDept && (currentDept.deptLevel || 0) > Number(deptLevel)) {
                // 查找父级部门
                const currentDeptParentCode = currentDept.parentCode;
                const parentDept = taskRes.data.find(
                  (dept) => dept.deptCode === currentDeptParentCode
                );
                if (parentDept) {
        ...
        const buildTree = () => {
          ...
          return deptList.filter(
            (dept) => dept.parentCode === topLevelDeptCode
          );
        };
                  ...
                  <>
                    ...
                    <Form.Item noStyle shouldUpdate>
                      {({ getFieldValue }) => (
                        <>
                          <p>
                            <strong>业务专题:</strong> {getFieldValue("businessTopics")?.length || 0} 个
                          </p>
                          <p>
                            <strong>责任单位:</strong> {getFieldValue("departments")?.length || 0} 个
                          </p>
                        </>
                      )}
                    </Form.Item>
              ...
              <Form.Item noStyle shouldUpdate>
                {({ getFieldValue }) => (
                  <>
                    <p>
                      <strong>业务专题:</strong>{" "}
                      {getBusinessTopicNames(selectedHistoryItem.businessTopics).join(
                        ", "
                      )}
                    </p>
                    <p>
