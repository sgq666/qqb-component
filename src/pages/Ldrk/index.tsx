...
const Ldrk: React.FC = () => {
    ...
    reader.onload = (e) => {
      try {
        ...
        headers.forEach((header, index) => {
          ...
          if (
            header &&
            (header.includes("身份证") || header.includes("证件"))
          ) {
            idCardIndex = index;
          }
        });
        ...
        if (idCardIndex === -1) {
          message.error(
            "未找到身份证号码列，请确保Excel文件包含'身份证'或'证件'列"
          );
          setLoading(false);
          return;
        }
  ...
  const recordColumns = [
    ...
    },
    {
      ...
      render: (_: any, record: ImportRecord) => {
        const percent =
          record.total > 0
            ? Math.round((record.successCount / record.total) * 100)
            : 0;
        return (
          <Progress
            percent={percent}
    ...
    },
    {
      ...
      render: (_: any, record: ImportRecord) => {
        const isCompleted =
          record.total > 0 && record.successCount === record.total;
        // 检查是否在20分钟限制期内
        const now = Date.now();
        ...
        const remainingTime = isViewDisabled
          ? Math.ceil((twentyMinutes - timeDiff) / 60 / 1000)
          : 0;
  ...
  const handleExportAllData = async () => {
    if (!selectedRecordUuid) {
      message.warning("请先选择一个导入记录");
      return;
    }

    try {
      // 显示加载提示
      const loadingMessage = message.loading("正在获取全部数据...", 0);

      // 调用后台接口获取全部数据
      const response = await ldrkService.getImportDetails(selectedRecordUuid);
  ...
  const handleExportFilteredData = async () => {
    ...
    try {
      ...
      const record = importRecords.find((r) => r.uuid === selectedRecordUuid);
      const fileName = record ? record.fileName : "导出数据";
      ldrkService.exportDetailsToExcel(detailDisplayData, fileName);
      ...
      <Card>
        ...
        <Tabs defaultActiveKey="1">
          ...
          <TabPane tab="数据导入" key="1">
            <p>
              上传包含姓名和身份证号码的Excel文件，系统将自动提取数据并导入（只要身份证号码不为空就会被提取，姓名可以为空）
            </p>
          ...
          <TabPane tab="导入记录" key="2">
            <Row justify="space-between" style={{ marginBottom: "10px" }}>
              ...
              <Col>
                <Button onClick={fetchImportRecords} loading={recordsLoading}>
                  刷新记录
                </Button>
              </Col>
            </Row>
            ...
            {selectedRecordUuid && (
              <Card
                title={`导入详情 - ${
                  importRecords.find((r) => r.uuid === selectedRecordUuid)
                    ?.fileName || selectedRecordUuid
                }`}
